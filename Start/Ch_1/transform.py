...
sortedgrades = list(sorted(grades))
letters = list(map(toGrade, sortedgrades))
smallletters = list(map(toSmaller, letters))
print(smallletters)
...
def simpler(q):
    return {
        ...
        'time': dt.date.fromtimestamp(q['properties']['time']/1000)
}

simplified = list(map(simpler, results))
